...
CHUNKED_LABELS = '../chunks/fingerprints_{}_{}.npy'
CHECKPOINT_PATH = '../pretrained/moldeberta-{}-{}-contrastive_mlc'
...
class SMILESDataset(Dataset):
  def __init__(self, chunk_indices, dataset_name):
    self.chunk_indices = chunk_indices
    self.dataset_name = dataset_name

    self.current_chunk_idx = -1
    self.current_chunk_data = None
    self.current_chunk_label = None
...
class CLoss(nn.Module):
  ...
  def forward(self, z, y):
    ...
    sim_z = torch.matmul(z, z.T) / tau
    sim_y = torch.matmul(y, y.T) / tau

    mask = torch.eye(sim_z.size(0), device=sim_z.device).bool()
    sim_z = sim_z.masked_fill(mask, -1e9)
    sim_y = sim_y.masked_fill(mask, -1e9)

    targets = nn.functional.softmax(sim_y, dim=1)

    loss = -(targets * nn.functional.log_softmax(sim_z, dim=1)).sum(dim=1)
    return loss.mean()
...
def main(model_size, dataset):
  ...
  criterion = CLoss()
  optimizer = AdamW(list(model.parameters()) + list(criterion.parameters()), lr=learning_rate)
